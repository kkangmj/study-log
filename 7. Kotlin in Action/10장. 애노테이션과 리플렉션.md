...
- buildString
  ...
  - 인자는 수신 객체 지정 람다며, 수신 객체는 항상 StringBuilder임(5.5.2절).
...
- serializeObject
  ...
  - StringBuilder 확장 함수지만 이 맥락에서만 유효한 함수이기 때문에 private로 지정함.
...
- 지금 읽는 책이 17년도에 출판된 거라 그런지 jkid 예제 코드가 조금 상이함. 10장 정리본에서는 최신 버전의 jkid 코드로 작성함.
